...
export async function createNewUser({ data, companyId }, token) {
    try {
        const response = await fetch(`${apiPrefix}/workouts/register/${companyId}`, {
            method: 'POST',
...
export const fetchList = async ({ companyId }, token) => {
    try {
        const response = await fetch(`${apiPrefix}/workouts/list/${companyId}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json"
            }
...
export async function apiEditInfo({data, customerID}) {
    return ApiService.fetchData({
        url: `/workouts/update/${customerID}`,
        method: 'put',
        data,
    })
}
...
export async function apiDelete({data, customerID}) {
    return ApiService.fetchData({
        url: `/workouts/delete/${customerID}`,
        method: 'delete',
        data,
    })
}
